...
const ParkingDetails = () => {
    const {parkingSpace, setParkingSpace, pay, setPay} = useContext(spaceContext)
    ...
    const onClickPlay = async (id : any) => {
       ...
       await  axios.post(`https://httpstat.us/200`, {
        ...
        const index: any = parkingSpace.findIndex((item: any) => item.id === id)

        if(index !== ''){
            parkingSpace[index].carNumber = ''
            parkingSpace[index].available = true
        }
  ...
  return (
    <Box sx={{
     ...
     { pay ? <Box>
      ...
      <Typography >Total Time Spent :-  {timeInHour} : {timeInMinutes}</Typography>
     ...
     <Typography  variant='h6' >Payment Taken</Typography>
